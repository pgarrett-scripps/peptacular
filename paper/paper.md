...
Peptacular addresses these limitations by providing two complementary APIs: an object-oriented interface, and a functional interface. The functional API accepts serialized ProForma sequences, applies transformations using parallel processing, and returns serialized results. This design enables direct integration with pandas and polars dataframes without sacrificing computational performance. The object-oriented interface allows for direct manipulation of the annotation objetcs without the need for serialiazationor parsing, improving performance for more complex operations.
