...
_CHEM_FORMULA_MOD_VALS = [
    "Formula:C12H22O11",
    "Formula:[13C6]H12O6[12C-4]",
    "Formula:CHO",
    "Formula:C2H-5O",
]
...
_BASE_MODS = (
    _UNIMOD_LEVEL_BASE_MOD_VALS + _PSI_LEVEL_BASE_MOD_VALS + _DELTA_MASS_MOD_VALS
)
...
def _random_sequence(
    amino_acids: str, min_sequence_length: int, max_sequence_length: int
) -> str:
    """
    ...
    return "".join(
        choice(amino_acids)
        for _ in range(randint(min_sequence_length, max_sequence_length))
    )
...
def random_sequence(
    ...
    sequence_ambiguity: bool = True,
) -> str:
    """
    Generate a random sequence based on the compliance level.
    """
    if level == ProformaComplianceLevel.BASE:
        return _random_sequence(
            _BASE_AMINO_ACIDS, min_sequence_length, max_sequence_length
        )

    if level == ProformaComplianceLevel.LEVEL2:
        if sequence_ambiguity:
            return _random_sequence(
                _LEVEL2_AMINO_ACIDS, min_sequence_length, max_sequence_length
            )
        return _random_sequence(
            _LEVEL2_AMINO_ACIDS_WITHOUT_AMBIGUITY,
            min_sequence_length,
            max_sequence_length,
        )
...
def random_mod(
    level: ProformaComplianceLevel, count: int = 1, info: bool = False
) -> Mod:
    """
...
def random_interval(
    level: ProformaComplianceLevel, start_index: int, end_index: int
) -> Interval:
    """
...
def random_intervals(
    ...
    for i in range(num_intervals):
        ...
        end_index = (
            start_index + segment_length - 1 if i < num_intervals - 1 else len(sequence)
        )
...
def compliance_randomizer(
    ...
    sequence = random_sequence(
        level, min_sequence_length, max_sequence_length, sequence_ambiguity
    )
...
def top_down_randomizer(annotation: ProFormaAnnotation, mod_prob: float = 0.1):
    ...
    internal_mods = [
        random_mod(
            level=ProformaComplianceLevel.TOP_DOWN, count=1, info=choice([True, False])
        )
        for _ in range(num_internal_mods)
    ]
...
def cross_linking_randomizer(annotation: ProFormaAnnotation):
    ...
    # Cross-linking
    if choice([True, False]):
        cross_link_mod = random_mod(
            level=ProformaComplianceLevel.CROSS_LINKING, count=1, info=False
        )
...
def glycan_randomizer(annotation: ProFormaAnnotation, mod_prob: float = 0.1):
    ...
    internal_mods = [
        random_mod(
            level=ProformaComplianceLevel.GLYCAN, count=1, info=choice([True, False])
        )
        for _ in range(num_internal_mods)
    ]
    internal_mod_indices = sample(range(len(annotation)), num_internal_mods)
