...
def parse_glycan_formula_db(formula: str, sep: str) -> ChemComposition:
    ...
    while formula != "":
        for glycan_name in MONOSACCHARIDES_DB.names_sorted:
            ...
            raise InvalidGlycanFormulaError(
                original_formula, f'Unknown glycan: "{formula}"!'
            )
...
def _get_resid_entries(terms: List[Dict[str, Any]]) -> List[ModEntry]:
    for term in terms:
        ...
        if len(definition) > 1:
            warnings.warn(
                f"[{DbType.RESID}] Multiple definitions for {term_id} {term_name} {definition}"
            )
        ...
        if len(delta_composition) > 1:
            warnings.warn(
                f"[{DbType.RESID}] Multiple delta compositions for {term_id} {term_name} {delta_composition}"
            )
            delta_composition = delta_composition[0]
        elif len(delta_composition) == 1:
            delta_composition = delta_composition[0]
...
def _get_xlmod_entries(terms: List[Dict[str, Any]]) -> List[ModEntry]:
    for term in terms:
        ...
        delta_composition = property_values.get(
            "bridgeFormula", property_values.get("deadEndFormula", [None])
        )
        ...
        if len(delta_composition) > 1:
            warnings.warn(
                f"[{DbType.XLMOD}] Multiple delta compositions for {term_id} {term_name} {delta_composition}"
            )
            delta_composition = delta_composition[0]
        elif len(delta_composition) == 1:
            delta_composition = delta_composition[0]
        ...
        if delta_composition:
            ...
            try:
                ...
                warnings.warn(
                    f"[{DbType.XLMOD}] Error parsing {term_id} {term_name} {delta_formula}, {e}"
                )
                delta_formula = None
                delta_average_mass = None
...
def _get_gno_entries(terms: List[Dict[str, Any]]) -> List[ModEntry]:
    for term in terms:
        ...
        if len(val) > 1:
            warnings.warn(
                f"[{DbType.GNO}] Multiple delta compositions for {term_id} {term_name} {val}"
            )
            val = val[0]
        elif len(val) == 1:
            val = val[0]
...
def _get_monosaccharide_entries(terms: List[Dict[str, Any]]) -> List[ModEntry]:
    for term in terms:
        ...
        try:
            ...
            warnings.warn(
                f"[{DbType.MONOSACCHARIDES}]Error parsing {term_id} {term_name} {delta_formula}, {e}"
            )
            delta_formula = None
...
class EntryDb:
    ...
    def _add_entry_to_id_map(self, entry: ModEntry) -> None:
        if entry.id in self.id_map and self.id_map[entry.id] != entry:
            warnings.warn(
                f"[{self.entry_type}] Duplicate id: {entry.id}\n{entry}\n{self.id_map[entry.id]}"
            )
    ...
    def _add_entry_to_name_map(self, entry: ModEntry) -> None:
        if entry.name in self.name_map and self.name_map[entry.name] != entry:
            warnings.warn(
                f"[{self.entry_type}] Duplicate name: {entry.name}\n{entry}\n{self.name_map[entry.name]}"
            )
    ...
    def _add_entry_to_synonym_map(self, entry: ModEntry) -> None:
        ...
        for syn in entry.synonyms:
            if syn in self.synonym_map and self.synonym_map[syn] != entry:
                warnings.warn(
                    f"[{self.entry_type}] Duplicate synonym: {syn}\n{entry}\n{self.synonym_map[syn]}"
                )
    ...
    def reload_from_online(self, use_synonyms: bool = False) -> None:
        ...
        try:
            ...
            raise ImportError(
                "The requests module is required to download the obo file from the internet"
            ) from err
...
MONOSACCHARIDES_DB.reload_from_file(
    os.path.join(_obo_path, "monosaccharides_updated.obo")
)
