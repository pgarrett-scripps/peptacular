...
class DeBruijnGraph:
    ...
    def __init__(self, k: int):
        ...
        self._outgoing_cache: Dict[str, list] = {}  # node -> [(next_node, cumulative_prob), ...]
    ...
    def get_amino_acid_frequencies(self) -> Dict[str, float]:
        ...
        if self.total_amino_acids == 0:
            return {}
        return {aa: count / self.total_amino_acids for aa, count in self.amino_acid_counts.items()}
    ...
    def get_edge_probability(self, from_node: str, to_node: str) -> float:
        ...
        """
        total_from_node = sum(count for (f, t), count in self.edges.items() if f == from_node)
    ...
    def get_outgoing_edges(self, node: str) -> Dict[str, float]:
        ...
        """
        outgoing = {to_node: count for (from_node, to_node), count in self.edges.items() 
                    if from_node == node}
    ...
    def generate_sequence(self, length: int, start_kmer: str | None = None, ending_chars: str | None = None) -> str:
        """
    ...
    def generate_sequence_with_ending(self, length: int, ending_chars: str, 
                                    start_kmer: str | None = None, max_search_size: int | None = None) -> str:
        """
...
def generate_sequences_parallel(
    ...
    args_list = [
        (graph, random.randint(length_range[0], length_range[1]), start_kmer, ending_chars, random.randint(0, 2**32 - 1))
