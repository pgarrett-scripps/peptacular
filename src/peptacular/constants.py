...
AVERAGE_ATOMIC_MASSES: Final[dict[str, float]] = map_atomic_symbol_to_average_mass(
    _infos
)
...
ATOMIC_SYMBOL_TO_ISOTOPE_NEUTRON_OFFSETS_AND_ABUNDANCES: Final[
    dict[str, list[tuple[int, float]]]
] = map_atomic_number_to_comp_neutron_offset(_infos)
ATOMIC_SYMBOL_TO_ISOTOPE_MASSES_AND_ABUNDANCES: Final[
...
class IonType(StrEnum):
    ...
    """
    Automatic case-insensitive matching for IonType Enum.
    """
    ...
    def _missing_(cls, value):
        """Handle case-insensitive lookup when value doesn't match exactly."""
...
ISOTOPE_COMPONENT_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"([0-9]*)([A-Za-z]+)(-?\d*\.?\d*)"
)
CONDENSED_CHEM_FORMULA_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"([A-Z][a-z]*|e|p|n)(-?\d*\.?\d*)"
)
ADDUCT_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"([+-]?)(\d*)?([A-Za-z]{1,2}\d*\+?-?)"
)
...
toleranceTypeLiteral = Literal["ppm", "th"]
...
MatchModeLiteral = Literal["closest", "largest", "all"]
