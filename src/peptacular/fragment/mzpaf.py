...
class InternalFragment:
    ...
    @property
    def neutral_difference_from_yb(self) -> Optional[str]:
        """
        Calculate the neutral loss/gain from the default yb internal fragment.
        Returns None if this is already a yb fragment.
        """
        if self.n_terminal_cleavage == "y" and self.c_terminal_cleavage == "b":
            return None
        
        # Table from the specification showing differences from yb
        differences: dict[tuple[str, str], Optional[str]] = {
            ("x", "a"): None,  #  Default, no difference
            ("x", "b"): "-CO",
            ("x", "c"): "-CO+NH",
            ("y", "a"): "-CO",
            ("y", "b"): None,  # Default, no difference
            ("y", "c"): "+NH",
            ("z", "a"): "-CHNO",
            ("z", "b"): "-NH", 
            ("z", "c"): None,  # No difference
            ("a", "x"): None,  # No difference
            ("a", "y"): "+CO",
            ("a", "z"): "+CHNO",
            ("b", "x"): "+CO",
            ("b", "y"): None,  # No difference
            ("b", "z"): "+NH",
            ("c", "x"): "+CHNO",
            ("c", "y"): "+NH",
        ...
        key = (self.n_terminal_cleavage.lower(), self.c_terminal_cleavage.lower())
...
class FragmentAnnotation:
    ...
    ion_type: Union[PeptideIon, InternalFragment, ImmoniumIon, ReferenceIon, 
                   NamedCompound, ChemicalFormula, SMILESCompound, 
                   UnknownIon, PrecursorIon]
    
    # Optional components
    analyte_reference: Optional[int] = None
    is_auxiliary: bool = False
    neutral_losses: List[str] = field(default_factory=lambda: [])
...
class mzPAFParser:
    ...
    def parse_single(self, annotation_str: str) -> FragmentAnnotation:
        ...
        if groups.get('mass_error'):
            mass_error = MassError(
                float(groups['mass_error']),
                groups.get('mass_error_unit', 'Da')
            )
        ...
        analyte_ref = int(groups['analyte_reference']) if groups.get('analyte_reference') else None
    ...
    def _parse_ion_type(self, groups: dict[str, Optional[str]]) -> Union[PeptideIon, InternalFragment, ImmoniumIon, ReferenceIon, 
                                                     NamedCompound, ChemicalFormula, SMILESCompound, 
                                                     UnknownIon, PrecursorIon]:
        ...
        if groups.get('series'):
        ...
        elif groups.get('internal_start') is not None:  # Changed: check if not None instead of truthiness
            ...
            label = int(groups['unannotated_label']) if groups.get('unannotated_label') else None
            return UnknownIon(label=label)
        else:
            raise ValueError("Could not determine ion type")
    
    def _parse_isotope(self, isotope_str: Optional[str]) -> Optional[IsotopeSpecification]:
        ...
        try:
            ...
            for ann in annotations:
                ...
                if isinstance(ann.ion_type, InternalFragment):
                    ...
                    diff = frag.neutral_difference_from_yb
                    if diff:
                        print(f"    Difference from yb: {diff}")
