...
def get_losses(
    sequence: str, losses: List[Tuple[str, float]], max_losses: int
) -> Set[float]:
    """
...
def _build_fragments(
    ...
    for span in spans:
        ...
        applicable_losses = get_losses(
            sequence=base_unmod_sequence, losses=losses, max_losses=max_losses
        )
                ...
                for loss in applicable_losses:
                    for c in charges:
                        ...
                        if return_type == "fragment":
                            ...
                            frags.append(
                                (fragment_mz, get_label(ion_type, c, number, loss, iso))
                            )
    return frags
