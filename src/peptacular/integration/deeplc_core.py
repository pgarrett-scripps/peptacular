...
def predict_rt_deeplc(
    ...
    calibration: tuple[Sequence[str | ProFormaAnnotation], Sequence[float]]
    | None = None,
    model: Any | None = None,
    ...
    unique_df = unique_df.drop(
        columns=["tuple_key", "original_index", "is_bad"]
    ...
    preds = dlc.make_preds(
        seq_df=unique_df,
        calibrate=(calibration is not None and len(calibration[0]) > 0),
    )

    # Create mapping from (peptide, modifications) tuple to prediction
    unique_df["pred_rt"] = preds
    pred_map = dict(
        zip(unique_df["seq"] + "|" + unique_df["modifications"], unique_df["pred_rt"])
    )
