...
def apply_static_mods(
    ...
    mode: ModMode = "skip",
    return_type: ModBuilderReturnType = "str",
    ...
    if isinstance(nterm_mods, Dict) and len(nterm_mods) > 0:
        nterm_mods = (
            {k: fix_list_of_mods(v) for k, v in nterm_mods.items()}
            if nterm_mods
            else {}
        )
    ...
    if isinstance(cterm_mods, Dict) and len(cterm_mods) > 0:
        cterm_mods = (
            {k: fix_list_of_mods(v) for k, v in cterm_mods.items()}
            if cterm_mods
            else {}
        )
...
def _apply_variable_mods_rec(
    mods: Dict[ModIndex, List[List[Mod]]],
        ...
        for curr_mods in curr_list_of_mods:
            if annotation.has_internal_mods_at_index(index):
                if mode == "overwrite":
                    ...
                    updated_annotation.add_internal_mod(
                        index, curr_mods, False
                    )  # will overwrite
                    yield from _apply_variable_mods_rec(
                        mods, updated_annotation, index + 1, max_mod_count, mode
                    )
                elif mode == "append":
                    updated_annotation = annotation.copy()
                    updated_annotation.add_internal_mod(
                        index, curr_mods, True
                    )  # will append
                    yield from _apply_variable_mods_rec(
                        mods, updated_annotation, index + 1, max_mod_count, mode
                    )
...
def _variable_mods_builder(
    ...
    var_annotations = _apply_variable_mods_rec(
        new_mod_map, annotation, 0, max_mods + starting_mod_count, mode
    )
...
def apply_variable_mods(
    ...
    internal_mods = {
        k: remove_empty_list_of_list_of_mods(v) for k, v in internal_mods.items()
    }
    internal_mods = {k: v for k, v in internal_mods.items() if v}

    if isinstance(nterm_mods, Dict) and len(nterm_mods) > 0:
        nterm_mods = (
            {k: fix_list_of_list_of_mods(v) for k, v in nterm_mods.items()}
            if nterm_mods
            else {}
        )
    elif isinstance(nterm_mods, list) and len(nterm_mods) > 0:
    ...
    nterm_mods = {
        k: remove_empty_list_of_list_of_mods(v) for k, v in nterm_mods.items()
    }
    nterm_mods = {k: v for k, v in nterm_mods.items() if v}

    if isinstance(cterm_mods, Dict) and len(cterm_mods) > 0:
        cterm_mods = (
            {k: fix_list_of_list_of_mods(v) for k, v in cterm_mods.items()}
            if cterm_mods
            else {}
        )
    elif isinstance(cterm_mods, list) and len(cterm_mods) > 0:
    ...
    cterm_mods = {
        k: remove_empty_list_of_list_of_mods(v) for k, v in cterm_mods.items()
    }
    cterm_mods = {k: v for k, v in cterm_mods.items() if v}
        ...
        for regex_str, mods_list in nterm_mods.items():
            for mods in mods_list:
                ...
                if nterm_annot != annotation:
                    n_term_annotations.extend(
                        _variable_mods_builder(
                            nterm_annot, internal_mods, max_mods, mode=mode
                        )
                    )
            ...
            for mods in mods_list:
                ...
                for n_term_annot in n_term_annotations:
                    ...
                    if cterm_annot != n_term_annot:
                        var_annotations.extend(
                            _variable_mods_builder(
                                cterm_annot, internal_mods, max_mods, mode=mode
                            )
                        )
    ...
    var_annotations.extend(
        _variable_mods_builder(annotation, internal_mods, max_mods, mode=mode)
    )
