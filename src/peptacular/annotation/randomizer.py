...
def get_random_mod_component(require_composition: bool = True) -> str:
    ...
    match mod_type:
        ...
        case "mass":
            return m.as_tag_mass(monoisotopic=True, include_cv=choice([True, False])).serialize()
        case _:
...
def generate_random_proforma_annotation(
    ...
    return ProFormaAnnotation(
        ...
        intervals=generate_random_intervals(length, interval_probability=mod_probability, require_composition=require_composition) if generate_intervals else None,
        charge=charge,
    )
