...
def get_random_mod_component() -> str:
    ...
    match mod_type:
        ...
        case "mass":
            return m.as_tag_mass(monoisotopic=True, include_cv=True).serialize()
        case _:
...
def generate_random_intervals(
    seq_length: int, interval_probability: float = 0.3, mod_probability: float = 0.5
) -> list["Interval"]:
...
def generate_random_proforma_annotation(
    ...
    return ProFormaAnnotation(
        ...
        labile_mods=get_random_mod_dict(mod_probability)
        if include_labile_mods
        else None,
        unknown_mods=get_random_mod_dict(mod_probability)
        ...
        static_mods=generate_static_mods_dict(mod_probability)
        if include_static_mods
        else None,
        intervals=generate_random_intervals(
            length, interval_probability=mod_probability
        )
        if generate_intervals
        else None,
        charge=random_charge_state(0, 5)
        if randint(0, 1) == 1
        else random_charge_adduct(),
    )
