...
from peptacular.score import compute_fragment_matches, FragmentMatch, match_spectra_indexes
...
class TestScore(unittest.TestCase):
    ...
    def test_match_spectra(self):
        ...
        result = match_spectra_indexes(mz_spectrum1, mz_spectrum2, 0.2, 'th')
        ...
        mz_spectrum1 = [100, 200, 300]
        mz_spectrum2 = [99.9, 100.1, 200, 200.05, 300.1, 300.2]
        result = match_spectra_indexes(mz_spectrum1, mz_spectrum2, 1000, 'ppm')  # 0.1 ppm tolerance
        assert result == [(0, 2), (2, 4), (4, 6)]

        # Test with no matches
        mz_spectrum1 = [50, 150, 250]
        mz_spectrum2 = [100, 200, 300]
        result = match_spectra_indexes(mz_spectrum1, mz_spectrum2, 0.05, 'th')
        assert result == [None, None, None]

        # Test with empty spectra
        mz_spectrum1 = []
        mz_spectrum2 = [100, 200, 300]
        result = match_spectra_indexes(mz_spectrum1, mz_spectrum2, 0.1, 'th')
        assert result == []
