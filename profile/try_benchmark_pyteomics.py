...
def benchmark_single_sequence_workload():
    ...
    for seq in sequences:
        seq_times, seq_masses = time_function(
            lambda s=seq: pt.mass(s),
            iterations=iterations_per_seq,
            warmup=1
        )
    ...
    for seq in sequences:
        seq_times, seq_masses = time_function(
            lambda s=seq: mass.calculate_mass(proforma=s),
            iterations=iterations_per_seq,
            warmup=1
        )
...
def benchmark_batch_workload():
    ...
    for batch_size in batch_sizes:
        ...
        pept_seq_times, _ = time_function(
            lambda: [pt.mass(s) for s in sequences],
        ...
        pyto_times, pyto_masses = time_function(
            lambda: [mass.calculate_mass(proforma=s) for s in sequences],
            iterations=iterations,
            warmup=2
        )
        pyto_mean = statistics.mean(pyto_times)
        ...
        mass_diffs = [
            abs(float(m1) - m2) 
            for m1, m2 in zip(pept_masses[0], pyto_masses[0])
        ]
        max_diff = max(mass_diffs)
...
def benchmark_sequence_length_scaling():
    ...
    for length in lengths:
        ...
        sequences = [
            generate_random_proforma(length=length, mod_probability=0.2)
            for _ in range(iterations)
        ]
        
        # Time both implementations
        pept_times, _ = time_function(
            lambda i=0: pt.mass(sequences[i % len(sequences)]),
            iterations=iterations,
            warmup=5
        )
        
        pyto_times, _ = time_function(
            lambda i=0: mass.calculate_mass(proforma=sequences[i % len(sequences)]),
            iterations=iterations,
            warmup=5
        )
