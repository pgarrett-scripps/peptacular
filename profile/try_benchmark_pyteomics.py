...
print(pt.get_available_start_methods())
print(pt.get_start_method())
...
def benchmark_mass_calculation():
    ...
    sequences = [
        generate_random_proforma(length=random.randint(10, 30))
    ...
    speedup_parallel = (
        pyto_mean * num_sequences
    ) / pept_parallel_mean  # Compare total time

    # Check mass agreement
    mass_differences = [
        abs(m1 - m2) for m1, m2 in zip(all_masses_pept, all_masses_pyto)
    ]
    mass_differences_parallel = [
        abs(m1 - m2) for m1, m2 in zip(all_masses_pept_parallel, all_masses_pyto)
    ]
    ...
    print(f"\n{'=' * 70}")
    print("Mass Calculation Benchmark - Random ProForma Sequences")
    ...
    print(f"{'-' * 70}")
    print(
        f"{'Peptacular (sequential)':<25} {pept_mean:>13.2f}   {pept_median:>13.2f}   {pept_stdev:>13.2f}"
    )
    print(
        f"{'Peptacular (parallel)':<25} {pept_parallel_mean:>13.2f}   {pept_parallel_median:>13.2f}   {pept_parallel_stdev:>13.2f}"
    )
    print(
        f"{'Pyteomics':<25} {pyto_mean:>13.2f}   {pyto_median:>13.2f}   {pyto_stdev:>13.2f}"
    )
    print(f"{'-' * 70}")
    print("\nMass Agreement (vs Pyteomics):")
    print(f"  Sequential - Max diff: {max_diff:.6f} Da, Avg diff: {avg_diff:.6f} Da")
    print(
        f"  Parallel   - Max diff: {max_diff_parallel:.6f} Da, Avg diff: {avg_diff_parallel:.6f} Da"
    )
    ...
    if speedup_parallel > 1:
        print(
            f"  Parallel:   Peptacular is {speedup_parallel:.2f}x FASTER than Pyteomics (sequential) ✅"
        )
    else:
        print(
            f"  Parallel:   Pyteomics is {1 / speedup_parallel:.2f}x FASTER than Peptacular (parallel) ⚠️"
        )
