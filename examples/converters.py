...
def run():
    ...
    # Convert multiple sequences in parallel
    ip2_sequences = [
        'K.PEPTIDE.K',
        'K.PEP(phospho)TIDE.K',
        'R.SEQS[Phospho]UENCE.R',
        'K.PROT(oxidation)EIN.K',
        'R.FRAG(acetyl)MENT.R'
    ]

    # Automatically uses parallel processing for lists
    proforma_sequences = pt.convert_ip2_sequence(ip2_sequences)

    print("\nConverted IP2 sequences:")
    for ip2, proforma in zip(ip2_sequences, proforma_sequences):
        print(f"  {ip2:30s} -> {proforma}")
