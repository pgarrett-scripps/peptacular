...
def generate_monosaccharide_db():
    ...
    for term in terms:
        ...
        try:
            ...
            raise ValueError(
                f"Error parsing {term_id} {term_name} {mono} {delta_composition}"
            )
    ...
    with open(
        "../src/peptacular/data/monosaccharide/id_to_chem_formula.json", "w"
    ) as f:
        json.dump(id_to_composition, f)

    with open(
        "../src/peptacular/data/monosaccharide/id_to_isotopic_mass.json", "w"
    ) as f:
        json.dump(id_to_isotopic_mass, f)

    with open(
        "../src/peptacular/data/monosaccharide/id_to_average_mass.json", "w"
    ) as f:
